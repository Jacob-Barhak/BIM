...
//const double MAXDBL = 20000.0;
///////////////////// XINJU
const double MAXDBL = DBL_MAX;

					// largest possible distance
const ANNdist	ANN_DIST_INF	=  MAXDBL;
...
public:
	...
	virtual int annSearchR(
		ANNpoint		q,		// Query point
		//int				k,		// Maximum number of points
		//ANNidxArray		* nn_idx, // nearest neighbor array 
		//ANNdistArray	* dd,		// dist to near neighbors
		//int & nn_idx,
		//float & dd,
	...
	virtual void getBBox3d(ANNpoint &bmin, ANNpoint &bmax){
		for (int i=0; i<3; i++){
			bmin[i] = bnd_box_lo[i];
			bmax[i] = bnd_box_hi[i];
		}
