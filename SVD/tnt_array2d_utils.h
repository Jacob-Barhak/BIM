...
template <class T>
Array2D<T> ConvertTransMatrixToArray2D(T TransMatrix[])
{
...
template <class T>
T* ConvertArray2DToTransMatrix(const Array2D<T> A)
{
	T* TransMatrix = new T[16];
	
	int M=A.dim1(); // M should be 4
    int N=A.dim2(); // N should be 4
	
	int counter = 0;

	for (int i=0; i<M; i++)
		for (int j=0; j<N; j++)
        	TransMatrix[counter++] = A[i][j];
